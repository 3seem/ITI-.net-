...
// Create()'s GET overload isn't async because it does nothing but return an empty
// View() no database call and no await so there's no asynchronous work to wait on
// The POST overload calls AddAsync SaveChangesAsync which is why it needs async


//Part B
// This loads the student from the database instead of rendering an empty form
// because Edit's whole point is to show what already exists



//Part C
// If id were renamed to `studentId` while the route still says {id}, model
// binding (which matches by NAME, not position) would find nothing called
// `studentId` in the route values and default it to 0. Since student.Id came from
// the form and is the real id, the guard clause's `id != student.Id` check would
// now always be true  every edit would silently get rejected




//Part D
//[Range] is validation-only, not a schema attribute: changing its bounds never
// produces a pending EF Core migration, because it's never mapped to a database


//Part E
// This reloads the student from the database instead of reusing the POST's
// in-memory object because the in-memory object is only "what was typed" 
// it doesn't prove the save actually happened



//part F
//Lab ID 27. MIN_GPA_EDIT = 2.0 + (27 mod 5) × 0.3 = 2.0 + 0.6 = 2.6
//   MAX_YEAR_EDIT = (27 mod 3) + 2 = 0 + 2 = 2
//Route constraint(before any code runs) checks the shape of the URL.
//    Validation / ModelState (before the action's first line) checks the shape
//     of the data. Part D 
//    Database constraint (at save) checks the truth of the data.
//Before the save, the address bar held a POST to /Students/Edit, so F5 re-sent
//   that POST. After redirecting to MuhamadAssemConfirmed the address bar now holds a
//   GET to that action's address

//both [Required]/[MaxLength] and [Range] are written as plain C#
//attributes directly above a property. Different: [Required]/[MaxLength] are
//schema attributes — EF Core maps them onto the actual database columnboth [Required]/[MaxLength] and [Range] are written as plain C#
//attributes directly above a property. Different: [Required]/[MaxLength] are
//schema attributes — EF Core maps them onto the actual database column
 //while [Range] is validation-only never touches the database schema while [Range] is validation-only
 //  and never touches the database schema
...
namespace lab_17.Controllers
{
    public class StudentsController : Controller
    {
        ...
        public async Task<IActionResult> Edit(int id, Student student)
        {
            ...
            TempData["Message"] = $"{student.FullName} updated";

            //uncomment the next line to see part C 
            //return RedirectToAction("index");


            //part E
            return RedirectToAction("MuhamadAssemConfirmed", new { id = student.Id });
        }
        ...
        //Part E
        public async Task<IActionResult> MuhamadAssemConfirmed(int id)
        {
            var student = await _context.Students.FindAsync(id);
            if (student == null)
            {
